...
function Game() {
    ...
    this.sources.push(new Source(this, { x: this.size.x / 4 + fFX(), y: this.size.y / 4 + fFY() }));
    this.sources.push(new Source(this, {x: this.size.x / 4 * 3 + fFX(), y: this.size.y / 4 + fFY()}));
    this.sources.push(new Source(this, {x: this.size.x / 4 + fFX(), y: this.size.y / 4 * 3 + fFY()}));
    this.sources.push(new Source(this, {x: this.size.x / 4 * 3 + fFX(), y: this.size.y / 4 * 3 + fFY()}));
    // this.sources.push(new Source(this, {x: this.size.x / 2 + fFX(), y: this.size.y / 2 + fFY()}));
    ...
    //     // spawnRandomBody();
    //     // let distancePoints = [];
    //     // let sourcePairs = uniquePairs(this.sources);
    //     // sourcePairs.forEach(([sourceA, sourceB]) => {
    //         // console.log(sourceA.id, sourceB.id);
    //     for (var x = 0; x <= this.size.x; x++) {
    //         for (var y = 0; y <= this.size.y; y++) {
    //             // check if distance from point is equivalent for both sources, if so, add to distancePoints
